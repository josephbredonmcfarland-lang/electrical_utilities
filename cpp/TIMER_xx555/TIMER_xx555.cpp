...
// Signal Generator
std::vector<Sample> TIMER_xx555_MONOSTABLE::generate_signal(const std::vector<Sample>& input_signal) const {
        // Data
        ...
        for (size_t i = 1; i < input_signal.size(); ++i) {
            ...
            if (active_pulse && (current.time - pulse_start_time) >= pulsewidth()) {
                active_pulse = false;
            }
...
void TIMER_xx555_ASTABLE::set_r_b(float new_r_b) {
    r_b = new_r_b;
    validate_r_b();
}
// Calculators
float TIMER_xx555_ASTABLE::pulsewidth_low() const {
    // Output Pulse Low (s)
    return 0.693f*r_b*c;
}
float TIMER_xx555_ASTABLE::pulsewidth_high() const {
    // Output Pulse High (s)
    return 0.693f*(r_a+r_b)*c;
}
float TIMER_xx555_ASTABLE::pulsewidth() const {
    // Output Pulse Low (s)
    return pulsewidth_low() + pulsewidth_high();
}
float TIMER_xx555_ASTABLE::frequency() const {
    // Output Frequency (Hz)
    return 1/pulsewidth();
}
float TIMER_xx555_ASTABLE::dutycycle_driver() const{
    // Output Driver Duty Cycle
    return pulsewidth_low()/pulsewidth();
}
float TIMER_xx555_ASTABLE::dutycycle_waveform() const {
    return pulsewidth_high()/pulsewidth();
}
std::vector<Sample> TIMER_xx555_ASTABLE::generate_signal(float duration, float sample_rate) const {
    // Waveform Generators
        // Sample Timing
        if (duration <= 0) {
